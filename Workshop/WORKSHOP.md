# Steam Workshop package — Human Host Explosives

Everything needed to publish. **The upload itself needs your Steam account**, so that step is
yours; everything up to it is prepared here.

## Files to ship — VERIFIED against the game's own installer

App ID **2393970**. Human Host publishes mods through Steam Workshop and installs them itself;
`Human Host_Data/ModBrowser/Data/installed_mods.json` maps published-file IDs to installed files.

`Workshop/upload/` is the ready-to-publish item root. Build it with **`dotnet build -c Release`**
and copy `bin/Release/net472/HumanHostExplosives.dll` in - Release is optimized and emits no
`.pdb`, so no debug symbols ship. BepInEx loads a Debug build perfectly well, so this is about
tidiness rather than correctness.

`Workshop/upload/` is the ready-to-publish item root:

```
HumanHostExplosives/
  HumanHostExplosives.dll
  Grenade/grenade.obj, grenade.mtl, grenade.png, grenade_icon.png, preview.png
  Nailbomb/nailbomb.obj, nailbomb.mtl, nailbomb.png, nailbomb_icon.png
  Assets/Anim/throwanim.bundle
```

Refreshed 2026-09-06 (Release build) to include the nail bomb, which didn't exist yet when this
folder was first packaged — regenerate again after any future code or asset change, this folder is
a snapshot, not something the build keeps in sync automatically.

**Subfolders are supported** — the installer walks the item with
`Directory.GetFiles(workshopPath, "*", SearchOption.AllDirectories)` and copies each file to
`plugins/<relative path>`, creating directories as needed. Detection
(`SteamManager.HasModFilesInPlugins`) then checks the *same* relative path, so the layout must
match on both sides. Both confirmed by reading `SteamManager.dll`.

> **Use the named `HumanHostExplosives/` wrapper folder, not a flat root.** Paths are preserved
> relative to `plugins/` itself, so a flat item would scatter `Grenade/` and `Assets/` directly
> into the shared plugins directory — generic names, and a real collision risk with other mods.
> The wrapper installs to `plugins/HumanHostExplosives/...`, which is exactly the layout the mod
> already expects (it resolves assets relative to its own DLL) and matches how it runs in dev.
>
> Every currently-published mod for this game happens to be flat, because none of them ship
> assets — MiningDrill is a single 29 MB DLL with everything embedded. Flat is the convention,
> not a constraint.

Optional sidecars seen on published mods, neither required:

- `<name>.steampreview.png` — AutoRun ships one; a preview image for the in-game browser.
- `<name>.hhmm-i18n.json` — translated config-entry descriptions for the mod manager. Worth adding
  later if the mod gets traction; the format is a flat `{ "EntryName": { "de": "...", ... } }`.

BepInEx 5.4.23.2 is what the game's own installer fetches, so target that.

## Images

| File | Size | Use |
| --- | --- | --- |
| `workshop_thumbnail.png` | 512×512 | Workshop item thumbnail |
| `workshop_example.png` | 1280×720 | Description / preview image |
| `workshop_falloff_chart.png` | 1280×720 | Extra description image — damage-vs-distance falloff chart (grenade: 3,000 dmg/15m radius, nail bomb: 889 dmg/20m radius), current `.cfg` values |
| `workshop_falloff_thumb.png` | 512×512 | Square crop of the same chart, if a second thumbnail-sized image is ever needed |

Both `workshop_thumbnail.png`/`workshop_example.png` are regenerated by `tools/make_art.py` in the audit repo, from the mod's own item icons plus the game's real material icons pulled out of the Addressables bundles - so the recipes shown are the recipes the mod actually registers. Regenerate them whenever a recipe changes; they were last rebuilt 2026-09-06 for 2x Iron Ingot / 5x Gun Powder / 3x Duct Tape and 5x Nails / 5x Nitrate Powder / 3x Duct Tape.

The falloff chart images were rendered 2026-09-17 from an interactive HTML chart (headless-screenshotted, since Steam Workshop descriptions only support static BBCode `[img]` tags, not embedded HTML/JS) - regenerate by re-rendering that chart if `ExplosionDamage`/`NailbombDamage`/`ExplosionRadius`/`NailbombRadius` change in `Plugin.cs`.

> Worth adding before publishing: **a real in-game screenshot**. The generated example image is a
> clean feature card, but Workshop browsers respond to actual gameplay — a grenade mid-throw or a
> wall coming apart would carry more weight than anything composited. I can't capture that.

## Title

    Explosives — Grenades & Nail Bombs

## Short description

    Craftable frag grenades and improvised nail bombs, both with real throw animations. Damages
    players, creatures and buildings. Fully configurable.

## Description (Steam BBCode)

```
[h1]Explosives[/h1]
Craftable explosives for Human Host — with proper throw animations, real structural damage,
and everything tunable.

[h2]What it adds[/h2]
[list]
[*][b]Hand grenades[/b] you craft at the Gun Workbench, or find in loot. Uniform area damage
that ignores geometry — the refined, "boom now" option.
[*][b]Nail bombs[/b] you craft by hand, no workbench needed. A crude pipe bomb: fires a spread
of shrapnel that only damages what it can actually reach in a straight line, so cover matters.
Weaker against structures than a grenade, nastier against soft targets, and can cause bleeding.
[*][b]Hold to charge[/b] — throw distance scales with how long you hold the button.
[*][b]Real throw animations[/b] on the player character, with the game's own swing sound.
[*][b]Damage that lands[/b] — players, creatures, and buildings, through the game's own
structural damage systems, so walls break the way the game intends.
[/list]

[h2]Recipes[/h2]
[b]Grenade[/b] (Gun Workbench):
[list]
[*]2 × Iron Ingot — the casing
[*]5 × Gun Powder — the filler
[*]3 × Duct Tape — holds it together
[/list]
[b]Nail bomb[/b] (hand-crafted, no workbench):
[list]
[*]5 × Nails — the shrapnel
[*]5 × Nitrate Powder — a cruder filler than Gun Powder, made at the Biochemical Workbench
[*]3 × Duct Tape — holds it together
[/list]
Both use mid/late-game Chemistry products, so neither lands as a day-one item.

[h2]Loot[/h2]
Both items are added to existing loot tags rather than given a new spawn rate of their own, so
they turn up only where that kind of item already appears, at that rarity — and they respect
your loot-rate setting and looting skill automatically.

[h2]Configuration[/h2]
Everything is in [i]BepInEx/config/com.nf.humanhostexplosives.cfg[/i] — blast radius,
damage, self-damage, fuse, throw power, animation speed, release timing, sound, loot tags. Press
[b]F7[/b] in game to reload the config without restarting.

[h2]Requirements[/h2]
BepInEx 5. Singleplayer tested.

[h2]Known limits[/h2]
[list]
[*]Molotov is registered but disabled — waiting on a 3D model.
[*]Not tested in multiplayer.
[/list]
```

## Tags

`Items`, `Weapons`, `Crafting`, `Gameplay`

## Publishing

The game has a built-in mod browser and installs Workshop items itself, so publishing is the
normal Steam UGC flow for app **2393970**: upload the contents of `Workshop/upload/` as the item
content, with `workshop_thumbnail.png` as the preview.

Use SteamCMD's `workshop_build_item` with a VDF pointing `contentfolder` at
`Workshop/upload` and `previewfile` at `Workshop/workshop_thumbnail.png`, or whatever in-game
publish flow the ModBrowser exposes if it has one.

**The upload needs your Steam account, so that step is yours.** Everything up to it is prepared.

## 0.2.2 (2026-09-21) - ModMenu support

Registers a curated settings page with ModMenu (Workshop item 3794527105, GUID
`humanhost.modmenu`) via `ModMenuBridge.cs` - a soft dependency, so the mod is unchanged when
ModMenu is not installed. ModMenu.dll is referenced at compile time only (`Private=false`) and
must NOT be bundled in this item. Numeric settings were given `AcceptableValueRange`s so they
render as sliders. Change note for the upload: `Workshop/changenote-0.2.2.txt`. Pattern documented in
`HumanHost Audit/MOD_CONVENTIONS.md` #38.
Also ships `<Dll>.hhmm-i18n.json` (generated by `HumanHost Audit/tools/gen_hhmm_sidecars.py`) so
the Human Host Mod Manager (Workshop 3740140784) shows the same labels and short descriptions;
see MOD_CONVENTIONS.md #39.
